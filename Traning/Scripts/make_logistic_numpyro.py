...
def train_model(X_train, y_train, fold_i, repeat_i, name,
                num_samples=2000, num_warmup=2000, seed=0,
                save_idata=True):
...
def nested_cross_bayesian_logistic(
    ...
    for repeat_i in range(Number_CV):
        ...
        for fold_i, (train_ix, test_ix) in enumerate(skf.split(X, y)):
            ...
            scale_means.append(scaler.mean_)
            scale_std.append(scaler.scale_) 
            X_train_j = jnp.asarray(X_train_imp)
            X_test_j  = jnp.asarray(X_test_imp)
            y_train_j = jnp.asarray(y_train_raw.values)
            y_test_j = jnp.asarray(y_test_raw.values)
            ...
            fold_baseline = fold_prev * (1 - fold_prev)
            ...
            train_arr["bss_train"][j] = 1.0 - train_arr["brier_train"][j] / fold_baseline
            test_arr["bss_test"][j]   = 1.0 - test_arr["brier_test"][j]   / fold_baseline
            ...
            pooled_oof_bss_train.append(1 - brier_score_loss(y_train_raw.values, train_probs) / fold_baseline)
            ...
            pooled_oof_bss_test.append(1 - brier_score_loss(y_test_raw.values, test_probs) / fold_baseline)
